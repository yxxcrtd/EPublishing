...
//        	console.log(this.oa.length);
        	...
        	// 试读比例也是限制
        	var pN = $.trim($('#pageEnableNum').val());
            return pN && 0!=pN? pN : this.oa.length ; 
    ...
    g.prototype = {Bb: function() {
        ...
        Rb: function(c, d, e, f) {
            ...
            if (!window.annotations && eb.touchdevice && !f) {
                ...
                // 2015-11-25 hengrun.yuan Start
				if (!window.annotations && (false && !f) && (l = m), g = this.rf != n && this.rf[c.pageNumber] != n, c.ba != M) {
				// 2015-11-25 hengrun.yuan end
                    ...
                    if (d && 0 == q) {
                        ...
                        if (this.ab[k] != n) {
							...
							for (var ba = 0; G = this.ab[k][ba++]; ) {
								...
								// 2015-11-25 hengrun.yuan start
								var XX = n ;
								-1 == D.indexOf("actionGoToR") && -1 == D.indexOf("actionGoTo") && -1 == D.indexOf("actionURI") && (XX = D.match(/(https?|ftp):\/\/[^\s\/$.?#].[^\s]*/gi)) && (D = "actionURI(" + XX[0] + "):" + XX[0]);	
								// 2015-11-25 hengrun.yuan end
                    ...
                    for (var l = f.Ma[k].fe, g = "", q = 0, p = 0, t = -1, u = -1, z = d.split(" "), w = 0; w < l.length; w++) {
                        ...
                        if (0 == d.indexOf(g + x) && (g + x).length <= d.length && " " != g + x) {
                            if (g += x, -1 == t && (t = q, u = q + 1), d.length == x.length && (t = q), g.length == d.length) {
                                if (p++, window.ed == p) {
                                    if (c.ba == B || c.ba == U) {
                                        eb.browser.jb.rb ? jQuery("#pagesContainer_" + f.ia).scrollTo(jQuery(f.Ma[k].Za[t].el), 0, {axis: "xy",offset: -30}) : jQuery("#pagesContainer_" + f.ia).data("jsp").scrollToElement(jQuery(f.Ma[k].Za[t].el), r);
                                    }
                                    for (var F = t; F < q + 1; F++) {
                                        c.ba == f.Oa(c) ? (x = jQuery(f.Ma[k].Za[F].el).clone(), f.kb(c).jh(f, c, x, d)) : (jQuery(f.Ma[k].Za[F].el).addClass("flexpaper_selected"), jQuery(f.Ma[k].Za[F].el).addClass("flexpaper_selected_default"), jQuery(f.Ma[k].Za[F].el).addClass("flexpaper_selected_searchmatch"));
                                    }
                                } else {
                                    g = "", t = -1;
                                }
                            ...
                            if (0 <= (g + x).indexOf(z[0])) {
                                ...
                                if (window.ed == p && 0 < g.length) {
                                    ...
                                    x.css("left", G + J * I + "px");
                                    if (c.ba == B || c.ba == U) {
                                        jQuery(c.va).append(x), eb.browser.jb.rb ? jQuery("#pagesContainer_" + f.ia).scrollTo(x, 0, {axis: "xy",offset: -30}) : jQuery("#pagesContainer_" + f.ia).data("jsp").scrollToElement(x, r);
                                    }
                                    c.ba == f.Oa(c) && f.kb(c).jh(f, c, x, d);
                                    c.ba == U && jQuery("#dummyPage_0_" + f.ia + "_textoverlay").append(x);
                                    c.ba == C && (0 == k ? jQuery("#dummyPage_0_" + f.ia + "_1_textoverlay").append(x) : jQuery("#dummyPage_" + (k - 1) % 2 + "_" + f.ia + "_" + ((k - 1) % 2 + 1) + "_textoverlay").append(x));
            			...
            			+ (c.aa.config.document.ViewModeToolsVisible ? 
            					...
            					+ (c.aa.config.document.ZoomToolsVisible ? String.format("<img class='flexpaper_tbbutton_large flexpaper_bttnZoomIn' src='{0}' style='margin-left:5px;' />", c.resources.pa.oj) + String.format("<img class='flexpaper_tbbutton_large flexpaper_bttnZoomOut' src='{0}' style='margin-left:-1px;' />", c.resources.pa.pj) 
            							+ (!eb.platform.Nb ? String.format("<img class='flexpaper_tbbutton_large flexpaper_bttnFullScreen' src='{0}' style='margin-left:-1px;' />", c.resources.pa.Ti) : "") + "" : "") + (c.aa.config.document.NavToolsVisible ? String.format("<img src='{0}' class='flexpaper_tbbutton_large flexpaper_previous flexpaper_bttnPrevPage' style='margin-left:15px;'/>", c.resources.pa.Hi) 
            									+ String.format("<input type='text' class='flexpaper_tbtextinput_large flexpaper_currPageNum flexpaper_txtPageNumber' value='1' style='width:70px;text-align:right;' />") + String.format("<div class='flexpaper_tblabel_large flexpaper_numberOfPages flexpaper_lblTotalPages'> / </div>") + String.format("<img src='{0}'  class='flexpaper_tbbutton_large flexpaper_next flexpaper_bttnPrevNext'/>", c.resources.pa.Ji) + "" : "") + (c.aa.config.document.SearchToolsVisible ? String.format("<input type='text' class='flexpaper_tbtextinput_large flexpaper_txtSearch' style='margin-left:15px;width:130px;' />") + String.format("<img src='{0}' class='flexpaper_find flexpaper_tbbutton_large flexpaper_bttnFind' style=''/>", c.resources.pa.Qi) + "" : "")), jQuery(c.ca).addClass("flexpaper_toolbarios"));
        }
...
window.FlexPaperViewer_Zine = function(g, c, d) {
    ...
    this.Hj = function(c) {
        ...
        d.Rc = d.aa.Fd ? jQuery("#" + d.Ja).wrap("<div id='" + d.Ja + "_wrap' style='text-align:center;width:100%;position:absolute;z-index:100;top:-70px;background-color:#258FD4'></div>").parent() : jQuery("#" + d.Ja).wrap("<div id='" + d.Ja + "_wrap' style='text-align:center;width:100%;background-color:#258FD4'></div>").parent();
        d.aa.ug = m;
    ...
    this.fh = function() {
    	...
    	jQuery(this.toolbar.ca).css("background-color",'#258FD4');
                ...
                if (e.ba == M) {
                    ...
                    if (0 == e.renderer.Ra.Ma.length && 10 > e.yf) {
                        ...
                        for (; window.Ua - 1 < g; ) {
                            ...
                            if (0 <= window.Vb) {
                                e.sa != window.Ua && (e.ba == e.ra() && e.sa != window.Ua + 1 || e.ba == C && e.sa != window.Ua + 1 || e.ba == H && e.sa != window.Ua - 1 || e.ba == U && e.sa != window.Ua) && (e.ba == H || e.ba == C || e.ba == U || e.ba == e.ra()) ? e.gotoPage(window.Ua, function() {
                                    window.Vb -= c.length;
                                    e.searchText(c);
