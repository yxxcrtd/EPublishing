...
function onCurrentPageChanged(pagenum){
	// <试读> 试读到最后一页弹出推荐框
	/*if($('#pageEnableNum').val() && $('#pageEnableNum').val() != "0" && $('#pageEnableNum').val()>0 ){
		if(pagenum && pagenum == $('#pageEnableNum').val()){
			endPageFlag++  ; 
			$('#documentViewer').bind("mouseup",function(e){
				recommends_dialog($("#pubId11").val());
			});
		}else if(endPageFlag > 0){
			$("#documentViewer").unbind("mouseup");
			endPageFlag = 0 ; 
		}
		return ;
	}*/
