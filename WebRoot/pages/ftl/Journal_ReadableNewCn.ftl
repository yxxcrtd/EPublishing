	                    ...
	                    <li class=featureBox style="height: 590px;">
		                    ...
		                    <#list list as p>
								...
								<#else>
									<a href="${request.contextPath}/pages/publications/form/article/${p.publications.id}">
										<div class="imgMoren">
										<p><#if (8 < p.publications.title?length)>${p.publications.title[0..7]}...<#else>${p.publications.title}</#if></p>
										</div>
									</a>
								</#if>
									<p class="bookTit"><a class="a_title" title="${(p.publications.title)!}" href="${request.contextPath}/pages/publications/form/article/${p.publications.id}">
									<#if (8 < p.publications.title?length)>${p.publications.title[0..7]}...<#else>${p.publications.title}</#if></a></p>
								</div>
