		...
		<c:if test="${p.startPage>0}">
			 <c:choose>
			 	<c:when test="${not empty pageEnableNum && p.startPage > pageEnableNum }">
			 		<li class=" ${p.fullText}" style="margin-left:${p.browsePrecent}px"><span name="toc_span_name">${p.title }</span></li>
			 	</c:when>
			 	<c:otherwise>
					<li class=" ${p.fullText}" style="margin-left:${p.browsePrecent}px;color:#258FD4;"><a   href="javascript:void(0)" onclick="getDocViewer().gotoPage(${p.startPage==null?1:p.startPage})" title="${p.title }">${p.title }</a></li>
			 	</c:otherwise>			 
			 </c:choose>
		</c:if>
		<c:if test="${p.startPage<=0}">
