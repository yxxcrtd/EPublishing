...
<link rel="shortcut icon"
	href="/ark/pics/favicon/favicon.ico?v=7728138265" type="image/x-icon">
...
<body>
	...
	<div style="display: none; color: #000;" id="ark-reader" class="">
		...
		<script id="tmpl-panels-container" type="text/template"><a href="#" class="close">&times;</a><section class="panel-content"><div class="chapters-toc controls-content"></div><div class="toc controls-content"></div><div class="bookmarks controls-content"></div><div class="annotations controls-content"></div><div class="searchs controls-content"></div></section></script>
		...
		<script id="tmpl-controls-panel" type="text/template">
			<ul class="controls-list">
				<li class="controls-item toggle-chapters-toc" data-helper="篇目" title="篇目"><b class="arkicon-chapters-list"></b></li>
				<li class="controls-item toggle-toc" data-helper="目录" title="目录"><b class="arkicon-list"></b></li>
				<li class="controls-item toggle-searchs" data-helper="搜索" title="搜索"><span class="arkicon-searchs"></span></li>
				<li class="controls-item toggle-bookmarks" data-helper="我的书签" title="我的书签"><b class="arkicon-bookmarks"></b></li> {{ var toggleAnnotationsLabel = isGallery ? '我的批注' : '批注和划线'}} 
				<li class="controls-item toggle-annotations" data-helper="{{- toggleAnnotationsLabel}}" title="{{- toggleAnnotationsLabel}}"><b class="arkicon-annotations"></b></li>
			</ul>
		...
		<script id="tmpl-panel" type="text/template"><ul class="panel"> {{ if (!isAnonymous) { }}  {{ } }} <li id="fn-layout" class="sep" data-helper="阅读模式"><a href="#" title="阅读模式" class="arkicon-layout {{ if (layout === 'vertical') { }}vertical{{ } }}"></a></li>
		     <!--<li id="fn-salon" data-helper="评论" {{ if (!canRate) { }} class="sep" {{ } }}><a href="{{- reviewsUrl}}" title="评论" target="_blank" class="arkicon-salon"></a></li> 
		     {{ if (canRate) { }} <li id="fn-rating" data-helper="评分"><a href="#" title="评分" target="_blank" class="arkicon-star"></a></li> {{ } }} {{ if (!isAnonymous) { }} 
                <!--<li id="fn-share" class="sep page-sharing" data-title="分享"><a href="#" title="分享" class="arkicon-share"></a></li>
               {{ } }} <li id="fn-question" data-helper="提问"><a href="{{- questionsUrl }}" title="提问" target="_blank" class="arkicon-question"></a></li><li id="fn-comment" data-helper="讨论"><a href="{{- reviewsUrl }}" title="讨论" target="_blank" class="arkicon-salon"></a></li><li id="fn-favor" data-helper="喜欢" class="sep"><a href="#" title="喜欢" target="_blank" class="arkicon-favor"></a></li> --><!--取消评论按钮和分享按钮-->
		     {{ if (!isGallery) { }}  
		     {{ } }} 
		...
		<script id="tmpl-note-display" type="text/template"><div class="note-owner"><a href="{{- note.owner.url}}" target="_blank"><img class="avatar" src="{{- note.owner.avatar}}" alt="{{- note.owner.name}}"></a><span class="author"><a href="{{- note.owner.url}}" class="lnk-author" target="_blank">{{- note.owner.name}}</a> {{ if (isArticleAuthor) { }} <span class="label-author">（作者）</span> {{ } }} 的批注 </span></div><div class="note-content">{{= autoLink(note.note)}}</div><div class="actions"><span class="private-info">仅自己可见&nbsp;|</span> {{ var split = ''}} {{ var dot = '·'}} {{ if ('edit' in actions) { }} {{- split}} <a class="edit" href="#">修改</a> {{ split = dot }} {{ } }} {{ if ('delete' in actions) { }} {{- split}} <a class="delete" href="#">删除</a> {{ split = dot }} {{ } }} {{ if ('comment' in actions) { }} {{- split}} <a class="comment" href="#"></a> {{ split = dot }} {{ } }} {{ if ('favorite' in actions) { }} {{- split}} <a class="favorite" href="#"></a> {{ split = dot }} {{ } }} 
             <!--{{ if ('share' in actions) { }} <span class="share-wrapper"> {{- split}} <a class="share" href="#">推荐</a> {{ split = dot }} </span> -->
	    ...
	    <script id="tmpl-searchs-panel" type="text/template">
			<div class="searchs-hd"><h2>搜索</h2></div>
			<!--<div class="searchs-bd">-->
				<div class="searchs-box">
					<p><input type="text" class="key-word"></p>
					<p><a href="#" class="search-content">
						<b class="arkicon-zoom-out"></b>
						</a>
					</p>
				</div>
				<div class="searchs-list">
					<ul class="firstUl">
						<li><span><a href="#" class="on-cur"></a></span>
							<ul class="cur-ul-dis">
								<li><span class="sea-cont"><a hrer=""></a></span><span class="sea-num"><a></a></span></li>								
							</ul>
						</li>
...
<!-- <script id="tmpl-purchase-guide" type="text/template"><div class="item">试读已结束{{ if (device === 'iphone') { }}，{{ print(+price ? '购买' : '登录') }}后继续阅读{{ } else { }}。{{ print(+price ? '购买' : '登录') }}后，可以继续阅读。{{ } }}</div><div class="item purchase-section"></div><div class="item"><a href="/ebook/{{= id }}/" target="_blank" class="link-more">去商店了解更多</a></div></script>
 ...
 -->	<script id="tmpl-extra-controls" type="text/template">
  <!--{{ if (isSample && +price) { }}<div class="btn-purchase-wrapper"></div>{{ } }}{{ if (isAnonymous) { }}<a href="#" data-target-dialog="login" class="login">登录</a>{{ } else if (!hasAdded) { }}<a href="#" class="lnk-collect">加入阅读列表</a>{{ } }}-->
     </script>
	<script id="tmpl-chapter-extra-controls" type="text/template">
   <!--{{ if (isAnonymous) { }}<a href="#" data-target-dialog="login" class="login">登录</a>{{ } else if (!isSubscribed) { }}<a href="#" class="lnk-subscribe">订阅本{{= kind}}</a>{{ } }}-->
    </script>
...
{{ if (isAnonymous) { }}<a data-target-dialog="login" class="btn btn-primary login">订 阅</a>{{ } else if (!is_subscribed) { }}<a class="btn fixed-btn btn-primary btn-subscribe">订 阅</a>{{ } }}</script>
	...
	<script id="tmpl-sharing-form" type="text/template"><div class="bd">
	<!--<div class="textarea-shell"><textarea class="textarea-text"
        maxlength="140"
	...
	<script id="tmpl-note-form" type="text/template"><div class="textarea-shell"><textarea placeholder="你的批注" name="text" required>{{- content}}</textarea></div>
	<div class="actions"> {{= sharingActionsHtml}} <div class="form-actions"><a class="ln-cancel" href="#">取消</a><button class="btn btn-bubble btn-post" type="submit">确定</button></div></div>
	</script>
<script id="tmpl-note-inline-form" type="text/template"><textarea type="text" maxlength="500" class="text">{{- text}}</textarea><div class="form-footer"><div class="form-actions"><a href="#" class="button cancel">取消</a><button type="submit" class="btn post">保存</button></div><div class="private-setting"><label class="checkbox"><input type="checkbox" {{ if (visible_private) { print('checked') } }} name="visible_private" >仅自己可见 </label></div></div></script>
	<script
		src="${CONTENT_PATH }/ark/js/dist/reader/setup-8317154807.js"></script>
	<script>
		var Ark = {
...
MathJax.Hub.Config({config:["MMLorHTML.js"],extensions:["tex2jax.js","toMathML.js"],"HTML-CSS":{linebreaks:{automatic:!0,width:"95% container"}},jax:["input/TeX","output/NativeMML"],skipStartupTypeset:!0,"v1.0-compatible":!1,messageStyle:"simple",styles:{"#MathJax_MSIE_Frame":{left:"45%"},"#MathJax_Message":{top:"18.75em",left:"45%",color:"#999",border:"none",background:"none","font-size":"auto"}},showMathMenu:!1,showMathMenuMSIE:!1,positionToHash:!1,preRemoveClass:"mjp",errorSettings:{message:["[公式解析失败]"]}}),"simple"===MathJax.Hub.config.messageStyle&&(MathJax.Message.filterText=function(){var labelAlias={Loading:"加载资源文件...",Processing:"处理中...",Typesetting:"解析..."},progressAlias={"Typesetting math:":"处理数学公式:","Processing math:":"解析数学公式:"};return function(text){var alias,progressAliasMatch=text.match(/^\w+ math:/);if(progressAliasMatch&&(alias=progressAlias[progressAliasMatch[0]]))return text.replace(progressAliasMatch[0],alias);var labelAliasMatch=text.match(/^\w+/);return labelAliasMatch?labelAlias[labelAliasMatch[0]]:text}}());</script>
	<script>
				...
				function() {
					var arkenv = {
						me : {
							"name" : "jack",
							"user_type" : 1,
							"profileURL" : "http:\/\/${CONTENT_PATH }/epubReader\/people\/103726057\/",
		...
		define('mathjax', '${CONTENT_PATH }/'
				+ 'libs/MathJax/2.1.0/MathJax.js')
