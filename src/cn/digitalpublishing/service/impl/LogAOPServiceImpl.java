...
public class LogAOPServiceImpl extends BaseServiceImpl implements LogAOPService {
	 ...
	 */
	@Override
	public void addSynPageRecord(String content_id,JSONArray arr) throws Exception {
		try {
			...
			for(int i=0;i<size;i++){
				page = new PPage();
				JSONObject po = (JSONObject)arr.get(i);
				String page_id= po.getString("id");
				String page_num= po.getString("number");
				String page_pdf= po.getString("pdf");
				//String page_status= po.getString("sendStatus");
				String page_swf= po.getString("swf");
				String page_mark = po.getString("mark");
				String page_png = po.getString("png");
				page.setId(page_id);
				page.setMark(page_mark);
				page.setNumber(Integer.parseInt(page_num));
				page.setPdf(page_pdf);
				page.setPng(page_png);
				page.setSwf(page_swf);
				PPublications pub = new PPublications();
				pub.setId(content_id);
				page.setPublications(pub);
				//保存入库
				this.daoFacade.getpPageDao().insert(page);
			}		 
			
		} catch (Exception e) {
			throw new CcsException((e instanceof CcsException) ? e.getMessage()	: "page.add.error", e);//同步数据失败！
		}
	}
