...
public class StatisticsBookSuppliersImpl extends BaseServiceImpl implements StatisticsBookSuppliersService {
	...
	/**
	 * @param type 操作类                 1-访问摘要 2-访问内容 3-检索
	 * @parm  access 访问状态        1-访问成功 2-访问拒绝
	 * @param stype  检索类型        1-常规检索 , 2-高级检索 , 3-分类法点击
	 * 废弃
	 * 源程序问题：为什么不用sql汇总功能，而用程序实现汇总？？？？？？？？？？？？
	 * 
	 */
	public void getSupplierFast(Integer type, Integer access, Integer stype, String stimes, String year, String sourceid) throws Exception {
		...
		try {
			Map<String, Object> condition = new HashMap<String, Object>();
			BookSuppliersForm form = new BookSuppliersForm();
			sourceid ="ff8080813c69a8fd013c6a554b350253";
			if (sourceid != null) {//机构ID
				condition.put("isInstitutionId", sourceid);
			}
			condition.put("year", year);
			...
			System.out.println(condition+"-----------------"+list.size());
			if (list != null && list.size() > 0) {
				flag = true;
			}
			...
			if (flag) {
				for (LAccess o : list) {
					System.out.println(o.getInstitutionId()+"---"+o.getPublications().getId()+"--"+o.getPlatform()+"---"+o.getAccessDate());
	 ...
	 * @param stype   检索类型     type=2------1-没有License,2-超出并发数
	 *                        type=3------1-常规检索 , 2-高级检索 , 3-分类法点击
	 * @param month   月份
	 * @param year    年份
	 * @param institutionId 机构ID
	 * @param fulltextType 全文统计类型  1-试读,2-试用,3-购买资源
	 * @throws Exception
	 */
	@Override
	public void getSupplierFast_v2(Integer type, Integer access, Integer stype,
			String month, String year, String institutionId,Integer fulltextType) throws Exception {
		List<SSupplier> list = null;
		
		try {				
			Map<String, Object> condition = new HashMap<String, Object>();
			institutionId ="ff8080813c69a8fd013c6a554b350253";
			if (institutionId != null) {//机构ID
				condition.put("isInstitutionId", institutionId);
			}
			...
			switch (type) {
				case 2:
					if (stype != null) {	//1-没有License,2-超出并发数
						condition.put("refusedVisitType", stype);						
					}	
					break;
				case 3:
					if (stype != null) {	//1-常规检索 , 2-高级检索 , 3-分类法点击---！（标准检索 联邦检索）
						condition.put("searchType", stype);
					}
					break;
				default:
					break;
			}
			condition.put("accessFullType", fulltextType);	//全文统计类型  1-试读,2-试用,3-购买资源
			
			list = this.daoFacade.getSuppDao().getCounterList_v2(condition, "");			
								
			System.out.println(condition+"-----------------"+list.size());
			for(SSupplier obj : list){
							
				obj.setYear(Integer.parseInt(year));
				obj.setMonth(Integer.parseInt(month));
				obj.setInstitutionid(institutionId);
				...
				obj.setToc(0);
				obj.setDownload(0);
				obj.setSuppFullAccessTryRead(0);
				obj.setSuppFullAccessTryUse(0);
				obj.setSuppFullAccessBuy(0);
				
				
				int count = obj.getStaticCount();
				if (type == 1) {
					obj.setToc(count);
				} else if (type == 2 && access != null && access == 1) {
					obj.setFullAccess(count);
					//全文查看 1-试读,2-试用,3-购买资源
					if(fulltextType!=null){
						if(fulltextType==1){
							obj.setSuppFullAccessTryRead(count);
						}else if(fulltextType==2){
							obj.setSuppFullAccessTryUse(count);
						}else if(fulltextType==3){
							obj.setSuppFullAccessBuy(count);
						}
					}
				} else if (type == 2 && access != null && access == 2) {
					if (stype != null && stype == 1) {
				...
				//datetime
				//pubId
				condition.put("institutionId", institutionId);
				condition.put("datetime", year+"-"+month);
				condition.put("pubId", obj.getPubId());
				System.out.println(condition);
				List<SSupplier> supList = this.daoFacade.getSuppDao().getList(condition, "");
				
				/*存在记录更新，不存在则新增*/
				if (supList == null || supList.isEmpty()) {
					this.daoFacade.getSuppDao().insert(obj);
				} else {
					SSupplier queryBean = supList.get(0);
					
					queryBean.setFullAccess(obj.getFullAccess() > 0 ? obj.getFullAccess() : queryBean.getFullAccess());
					queryBean.setFullRefused(obj.getFullRefused() > 0 ? obj.getFullRefused() : queryBean.getFullRefused());
					queryBean.setRefusedLicense(obj.getRefusedLicense() > 0 ? obj.getRefusedLicense() : queryBean.getRefusedLicense());
					queryBean.setRefusedConcurrent(obj.getRefusedConcurrent() > 0 ? obj.getRefusedConcurrent() : queryBean.getRefusedConcurrent());
					queryBean.setSearch(obj.getSearch() > 0 ? obj.getSearch() : queryBean.getSearch());
					queryBean.setSearchStandard(obj.getSearchStandard() > 0 ? obj.getSearchStandard() : queryBean.getSearchStandard());
					queryBean.setSearchFederal(obj.getSearchFederal() > 0 ? obj.getSearchFederal() : queryBean.getSearchFederal());
					queryBean.setToc(obj.getToc() > 0 ? obj.getToc() : queryBean.getToc());
					queryBean.setDownload(obj.getDownload() > 0 ? obj.getDownload() : queryBean.getDownload());
					this.daoFacade.getSuppDao().update(queryBean, SSupplier.class.getName(), queryBean.getId(), null);
				}
			}
	...
	@Override
	public List<LLicense> getLicense(String userId, String pubId) throws Exception {
		Map<String, Object> condition = new HashMap<String, Object>();
		condition.put("userId", userId);
		condition.put("pubId", pubId);
		return this.daoFacade.getlLicenseDao().getLicense(condition, "");
	}
}
