...
public class CustomServiceImpl extends BaseServiceImpl implements CustomService {
	...
	@Override
	public void handleLicenseipinsert(BIpRange obj) throws Exception {
		try {
			// 2.获取机构订阅
			Map<String, Object> condition = new HashMap<String, Object>();
			condition.put("institutionId", obj.getInstitution().getId());
			condition.put("isTrail", "0");
			condition.put("level", 2);
			//List<LLicense> licenseList = this.daoFacade.getlLicenseDao().getList(condition, "");这个sql语句查询太慢了。下面是优化的sql语句 by zhoudong 2015-11-09
			List<LLicense> licenseList = this.daoFacade.getlLicenseDao().getlistLLicense(condition);
			// 3.获取机构IP
			condition.put("startip",obj.getStartIp().toString());//才添加的 by zhopudong 2015-11-16 添加的原因，增加一个Ip就单独进行关联关系。
			condition.put("endip", obj.getEndIp().toString());//才添加的 by zhopudong 2015-11-16 添加的原因，增加一个Ip就单独进行关联关系。
			...
			if (licenseList != null && !licenseList.isEmpty() && ipRangeList != null && !ipRangeList.isEmpty()) {
				for (LLicense license : licenseList) {
						...
						licenseIp.setEip(ipRangeList.get(0).getEip());
						licenseIp.setEndIp(ipRangeList.get(0).getEndIp());
						licenseIp.setSip(ipRangeList.get(0).getSip());
						licenseIp.setStartIp(ipRangeList.get(0).getStartIp());
