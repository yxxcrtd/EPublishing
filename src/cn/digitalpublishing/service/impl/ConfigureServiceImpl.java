...
//			}
	...
	@Override
	public void insertEBsource(EBSource obj) throws Exception {
		try {
			this.daoFacade.getEbsourceDao().insert(obj);
		} catch (Exception e) {
			throw new CcsException((e instanceof CcsException) ? ((CcsException)e).getPrompt()	: "publisher.info.insert.error", e);//获取出版商信息失败！
		}
	}
	...
	@Override
	public void updateEBsource(EBSource obj,String id,String[] properties) throws Exception {
		try {
			this.daoFacade.getEbsourceDao().update(obj, EBSource.class.getName(), id, properties);
		} catch (Exception e) {
			throw new CcsException((e instanceof CcsException) ? ((CcsException)e).getPrompt()	: "publisher.info.update.error", e);//获取出版商信息失败！
		}
	...
	@Override
	public EBSource getEBSource(String sourceId) throws Exception {
		// TODO Auto-generated method stub
		EBSource ebSource = null ; 
		try{
			ebSource = (EBSource) this.daoFacade.getEbsourceDao().get(EBSource.class.getName(), sourceId) ; 
		}catch (Exception e){
			throw e ; 
		}
		return ebSource;
