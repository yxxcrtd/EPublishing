...
public class SSupplierDao  extends CommonDao<SSupplier,String> {
	...
	/**
	 * chensr 2015-11-17 新增汇总模块，修改实现方式
	 * @param condition 	    条件
	 * @param sort            排序
	 * 
	 * @throws Exception
	 */
	public List<SSupplier> getCounterList_v2(Map<String, Object> condition, String sort) throws Exception {
		List<SSupplier> list = null;
		
		List<Object> con = new ArrayList<Object>();
		con.add(condition.get("year"));
		con.add(condition.get("month"));
		con.add(condition.get("type"));
		String sql = " SELECT A.pub_id as pubId,A.l_platform as platform,A.count as staticCount,p.PUB_TITLE as title,"
				+ "p.CONTENT_TYPE as type,p.PUB_AUTHOR as author,p.SISBN as isbn,p.PUB_EISSN as eissn,"
		        + "p.PUB_PISSN as issn,bp.PUBLISHER_NAME as pubName,p.PUB_LANG as lang,p.SOURCE_ID as sourceId"     
		        + " FROM (SELECT PUB_ID, l_platform, COUNT (L_ID) as count FROM epub_l_access "
				+ "	WHERE l_year = ? AND l_month = ? AND L_TYPE = ?  ";
		//1-访问成功 , 2-访问拒绝
		if(condition.get("access")!=null){
			sql += " AND L_ACCESS=? ";	
			con.add(condition.get("access"));
		}
		//机构ID
		if(condition.get("isInstitutionId")==null){
			sql += " AND L_INSID is null ";
		}else{
			sql += " AND L_INSID=? ";	
			con.add(condition.get("isInstitutionId"));
		}
		//没有License,2-超出并发数
		if(condition.get("refusedVisitType")!=null){		
			sql += " AND L_REFUSEDVISITTYPE=? ";	
			con.add(condition.get("refusedVisitType"));
		}
		//1-常规检索 , 2-高级检索 , 3-分类法点击---！（标准检索 联邦检索）
		if(condition.get("searchType")!=null){		
			sql += " AND L_SEARCHTYPE=? ";	
			con.add(condition.get("searchType"));
		}
		//全文统计类型  1-试读,2-试用,3-购买资源
		if(condition.get("accessFullType")!=null){		
			sql += " AND L_ACCESS_FULL_TYPE=? ";	
			con.add(condition.get("accessFullType"));
		}
			
		sql += " GROUP BY L_PLATFORM, PUB_ID";
		sql += ") A  join EPUB_P_PUBLICATIONS p ON A.pub_id = p.pub_id"
				+ "  join EPUB_B_PUBLISHER bp on bp.PUBLISHER_ID = p.PUBLISHER_ID ";	
		//System.out.println(sql);	
		try {
			//添加数据自动映射到bean
			String fields = "pubId,platform,staticCount,title,type,author,isbn,eissn,issn,pubName,lang,sourceId";
			list = super.hibernateDao.getListBySql(fields,sql ,con.toArray(), sort,SSupplier.class.getName());
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
		}
