...
//		}
	...
	/**
	 * chensr 2015-12-09  新增汇总模块，获取产品是否购买，修改实现方式
	 * @param condition 	    条件
	 * @param sort            排序
	 * 
	 * @throws Exception
	 */
	public List<OOrderDetail> getBuyBookList(Map<String, Object> condition, String sort) throws Exception {
		List<OOrderDetail> list = null;
		
		List<Object> con = new ArrayList<Object>();
		/*con.add(condition.get("productCode"));
		con.add(condition.get("status"));
		con.add(condition.get("userId"));*/
		
		String sql = " SELECT ORDER_ID as id"
				+ " from EPUB_O_ORDER_DETAIL where 1=1 ";
		//ISBN/ISSN
		...
		//1-未处理 2-已结算 3-已取消 4-处理中  废除
		//1-未处理 2-已付款未开通 3-已付款已开通 4-处理中 10-未付款已开通  99-已取消
		
		//if(condition.get("status")!=null){		
			sql += " AND (ODETAIL_STATUS =3 or ODETAIL_STATUS =10)  ";	
			//con.add(condition.get("status"));
		//}
		//用户ID
		...
		try {
			//添加数据自动映射到bean
			String fields = "id";
			list = super.hibernateDao.getListBySql(fields,sql ,con.toArray(), sort,OOrderDetail.class.getName());
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
		}
