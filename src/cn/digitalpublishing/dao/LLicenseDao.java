...
public class LLicenseDao extends CommonDao<LLicense, String> {
	...
	private Map<String, Object> getWhere(Map<String, Object> map) {
		 ...
		 * map.get("isTrail")!=null){ if(flag==0){
		 * whereString+=" where a.isTrial = ?"; flag=1; }else{
		 * whereString+=" and a.isTrial = ?"; }
		 * condition.add(map.get("isTrail")); }
		 */
		...
		if (CollectionsUtil.exist(map, "isCn") && !"".equals(map.get("isCn"))) {
			...
			Object[] types = new Object[] { "chs", "chn", "cht","中文" };
			String where_ = "";
			String _where = ")";
			String where = "";
			if (flag == 0) {
	...
	public List<LLicense> getLicense(Map<String, Object> condition, String sort) throws Exception {
		List<LLicense> list = null;
		
		List<Object> con = new ArrayList<Object>();
		con.add(condition.get("userId"));
		con.add(condition.get("pubId"));
		
		String sql = " SELECT PUB_CODE as code,LICENSE_IS_TRIAL as isTrial,LICENSE_STATUS as status,LICENSE_TYPE as type"     
		        + " FROM EPUB_L_LICENSE"
				+ "	WHERE U_ID = ? AND PUB_ID = ? and LICENSE_STATUS=1";			
		
		try {			
			//添加数据自动映射到bean
			String fields = "code,isTrial,status,type";
			list = super.hibernateDao.getListBySql(fields,sql ,con.toArray(), sort,LLicense.class.getName());
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
		}
	...
	@SuppressWarnings("unchecked")
	public List<LLicense> getlistLLicense(Map<String, Object> condition)throws Exception {
		List<LLicense> list = null;
		String hql = " from LLicense a left join a.user b left join a.user.institution c";
		String property = "complicating,isTrial,trialPeriod,readUrl,id,code,createdby,createdon,status,type,startTime,endTime,licenseUId,licensePwd,accessType,accessUIdType,user.id,user.name,user.institution.id,user.institution.name";
		String field = "a.complicating,a.isTrial,a.trialPeriod,a.readUrl,a.id,a.code,a.createdby,a.createdon,a.status,a.type,a.startTime,a.endTime,a.licenseUId,a.licensePwd,a.accessType,a.accessUIdType,b.id,b.name,c.id,c.name";
		Map<String, Object> t = this.getWhere(condition);
		try {
			list = super.hibernateDao.getListByHql(property, field, hql + t.get("where").toString(), ((List<Object>) t.get("condition")).toArray(), null, LLicense.class.getName());
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
		}
