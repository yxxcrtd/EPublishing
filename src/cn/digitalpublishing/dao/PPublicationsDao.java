...
public class PPublicationsDao extends CommonDao<PPublications, String> {
	...
	private Map<String, Object> getWhere(Map<String, Object> map) {
		 ...
		 * isInstitutionid
		 */
		if ((CollectionsUtil.exist(map, "isInstitutionid")) && map.get("isInstitutionid") != null
				&& !"".equals(map.get("isInstitutionid"))) {
			if (flag == 0) {
		 ...
		 * 分类法Code
		 */
		if (CollectionsUtil.exist(map, "subjectCode") && !"".equals(map.get("subjectCode"))
				&& map.get("subjectCode") != null) {
			if (flag == 0) {
		 ...
		 */
		if (CollectionsUtil.exist(map, "isLicense") && !"".equals(map.get("isLicense"))
				&& map.get("isLicense") != null) {
		 ...
		 * isLicense 1、在已订阅中查询 2 、未订阅中
		 */
		if (CollectionsUtil.exist(map, "isLicense2") && !"".equals(map.get("isLicense2"))
				&& map.get("isLicense2") != null) {
			String where1 = "";
			String where2 = "";
			String where3 = "";
		...
		if (CollectionsUtil.exist(map, "Subscribed") && !"".equals(map.get("Subscribed"))
				&& map.get("Subscribed") != null) {
			String where1 = "";
			String where2 = "";
			String where3 = "";
		...
		if (CollectionsUtil.exist(map, "prevOrder") &&map.get("prevOrder")!=null && !"".equals(map.get("prevOrder"))) {
			if (flag == 0) {
				whereString += " where (a.journalOrder is not null and a.journalOrder < ?)";
				flag = 1;
			} else {
				whereString += " and (a.journalOrder is not null and a.journalOrder < ?)";
			}
			condition.add(map.get("prevOrder"));
		}
		if (CollectionsUtil.exist(map, "nextOrder") &&map.get("nextOrder")!=null && !"".equals(map.get("nextOrder"))) {
			if (flag == 0) {
				whereString += " where (a.journalOrder is not null and a.journalOrder > ?)";
				flag = 1;
			} else {
			...
			condition.add(map.get("nextOrder"));
		}
		...
		if (CollectionsUtil.exist(map, "publisheridnotnull") && !"".equals(map.get("publisheridnotnull"))
				&& map.get("publisheridnotnull") != null) {
			if (flag == 0) {
		 ...
		 * 排除的图书类型
		 */
		if (CollectionsUtil.exist(map, "noChapter") && !"".equals(map.get("noChapter")) && map.get("noChapter") != null) {
			if (flag == 0) {
				whereString += " where a.type <> ?";
				flag = 1;
			} else {
			...
			condition.add(Integer.parseInt(map.get("noChapter").toString()));
		}
		...
		if (CollectionsUtil.exist(map, "isOaorFree") && !"".equals(map.get("isOaorFree"))
				&& map.get("isOaorFree") != null) {
		...
		if (CollectionsUtil.exist(map, "createOn") && !"".equals(map.get("createOn")) && null != map.get("createOn")) {
			if (0 == flag) {
				whereString += " WHERE a.createOn BETWEEN TO_DATE('" + DateUtil.getBeforeMonthStartDay(new Date())
						+ "', 'yyyy-MM-dd') AND TO_DATE(?, 'yyyy-MM-dd')";
				flag = 1;
			} else {
				whereString += " AND a.createOn BETWEEN TO_DATE('" + DateUtil.getBeforeMonthStartDay(new Date())
						+ "', 'yyyy-MM-dd') AND TO_DATE(?, 'yyyy-MM-dd')";
			}
	...
	public List<PPublications> getList(Map<String, Object> condition, String sort, CUser user, String ip)
			throws Exception {
		String sname = condition == null || condition.get("lang") == null
				|| "zh_CN".equals(condition.get("lang").toString()) ? "name" : "nameEn";
		...
		String property = "available,jsonPath,sourceId,pubDate,periodicType,journalType,keywords,treecode,order,activity,volume.id,issue.id,year,month,volumeCode,issueCode,chineseTitle,sisbn,hisbn,status,id,publisher.id,publisher.name,publisher.probationPer,publisher.probationValue,title,author,code,listPrice,lcurr,browsePrecent,type"
				+ ",pubSubject,cover,createOn,updateOn,pdf,path,lang,remark,homepage,newest,selected,special,pubDate,startPage,endPage,fileType"
				+ ",oa,free,local,homepage" + ",startVolume" + ",endVolume" + ",inCollection,latest,subscribedIp"
				+ ",publications.id " + ",publications.code " + ",publications.sisbn " + ",publications.hisbn "
				+ ",publications.title " + ",publications.pubSubject ";
		if (user != null) {
			property += ",subscribedUser,exLicense,buyInDetail,favorite";
		}
		property += ",recommand";
		property += ",pageNumCount,probation "; 
		String field = " a.available,a.jsonPath,a.sourceId,a.pubDate,a.periodicType,a.journalType,a.keywords,a.treecode,a.order,a.activity,a.volume.id,a.issue.id,a.year,a.month,a.volumeCode,a.issueCode,a.chineseTitle,a.sisbn,a.hisbn,a.status,a.id,b.id,b.name,b.probationPer,b.probationValue,a.title,a.author,a.code,a.listPrice,a.lcurr,a.browsePrecent,a.type"
				+ ",a.pubSubject,a.cover,a.createOn,a.updateOn,a.pdf,a.path,a.lang,a.remark,a.homepage,a.newest,a.selected,a.special,a.pubDate,a.startPage,a.endPage,a.fileType"
				+ ",a.oa,a.free,a.local,a.homepage" + ",a.startVolume" + ",a.endVolume" +
				...
				// +
		",a.inCollection"
				+ ",(case when exists(select pub.id from PPublications pub where pub.createOn >=? and pub.id=a.id) then '1' else '0' end) as isnewst"
				+
				// ",'0'";
				...
				+ ",c.id " + ",c.code " + ",c.sisbn " + ",c.hisbn " + ",c.title " + ",c.pubSubject";
		if (user != null) {
		...
		field += ",(select cast(count(*) as string) from PPage pp where pp.publications.id = a.id) as pageNumCount" ; 
		field +=", (CASE WHEN exists(SELECT bs.id FROM EBSource bs where bs.id=a.sourceId AND bs.probationStatus =1 ) THEN 1 else 0 end) as namesco";
		try {
			list = super.hibernateDao.getListByHql(property, field, hql + t.get("where").toString(), con.toArray(),
					sort, PPublications.class.getName());
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
		}
	...
	public List<PPublications> getLists(Map<String, Object> condition, String sort) throws Exception {
		...
		String property = " treecode,order,activity,volume.id,issue.id,year,month,volumeCode,issueCode,chineseTitle,sisbn,hisbn,status,id,publisher.id,publisher.name,title,author,code,listPrice,lcurr,browsePrecent,type"
				+ ",pubSubject,cover,createOn,updateOn,pdf,path,lang,remark,homepage,newest,selected,special,pubDate,startPage,endPage,fileType"
				+ ",oa,free,local,homepage" + ",startVolume" + ",endVolume,publications.id ";

		String field = " a.treecode,a.order,a.activity,a.volume.id,a.issue.id,a.year,a.month,a.volumeCode,a.issueCode,a.chineseTitle,a.sisbn,a.hisbn,a.status,a.id,b.id,b.name,a.title,a.author,a.code,a.listPrice,a.lcurr,a.browsePrecent,a.type"
				+ ",a.pubSubject,a.cover,a.createOn,a.updateOn,a.pdf,a.path,a.lang,a.remark,a.homepage,a.newest,a.selected,a.special,a.pubDate,a.startPage,a.endPage,a.fileType"
				+ ",a.oa,a.free,a.local,a.homepage" + ",a.startVolume" + ",a.endVolume,c.id ";

		try {
			list = super.hibernateDao.getListByHql(property, field, hql + t.get("where").toString(),
					((List<Object>) t.get("condition")).toArray(), sort, PPublications.class.getName());
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<PPublications> getList2(Map<String, Object> condition, String sort, CUser user, String ip)
			throws Exception {
		List<PPublications> list = null;
		...
		String property = "publications.id,publications.title,publisher.id,publisher.name,publisher.nameEn,id,title,chineseTitle,author,code,browsePrecent,type,year,pubDate,remark,pdf,cover,browsePrecent,startPage,endPage"
				+ ",authorAlias,oa,free,local,listPrice,lcurr,volumeCode,issueCode,lang,sisbn,pissn " + ",startVolume"
				+ ",endVolume" + ",inCollection,latest,subscribedIp";
		if (user != null) {
			property += ",subscribedUser,exLicense,buyInDetail,favorite";
		}
		property += ",recommand";// ,buyTimes";
		String field = "p.id,p.title,b.id,b.name,b.nameEn,a.id,a.title,a.chineseTitle,a.author,a.code,a.browsePrecent,a.type,a.year,a.pubDate,a.remark,a.pdf,a.cover,a.browsePrecent,a.startPage,a.endPage"
				+ ",a.author,a.oa,a.free,a.local,a.listPrice,a.lcurr,a.volumeCode,a.issueCode,a.lang,a.sisbn,a.pissn "
	...
	public List<PPublications> getSubscribedPagingList(Map<String, Object> condition, String sort, Integer pageCount,
		...
		String property = "volume.id,issue.id,year,month,volumeCode,issueCode,chineseTitle,sisbn,hisbn,status,id,publisher.id,publisher.name,title,author,code,listPrice,lcurr,browsePrecent,type"
				+ ",pubSubject,cover,createOn,updateOn,pdf,path,lang,remark,homepage,newest,selected,special,pubDate,startPage,endPage"
				+ ",oa,free,local" + ",startVolume" + ",endVolume" + ",inCollection,latest,subscribedIp";
		if (user != null) {
			property += ",subscribedUser,exLicense,buyInDetail,favorite";
		}
		property += ",recommand";
		String field = "a.volume.id,a.issue.id,a.year,a.month,a.volumeCode,a.issueCode,a.chineseTitle,a.sisbn,a.hisbn,a.status,a.id,b.id,b.name,a.title,a.author,a.code,a.listPrice,a.lcurr,a.browsePrecent,a.type"
				+ ",a.pubSubject,a.cover,a.createOn,a.updateOn,a.pdf,a.path,a.lang,a.remark,a.homepage,a.newest,a.selected,a.special,a.pubDate,a.startPage,a.endPage"
				+ ",a.oa,a.free,a.local" + ",a.startVolume" + ",a.endVolume" +
				// ",(case when exists(select cc.id from PCcRelation cc where
				// cc.publications.id=a.id and cc.collection.status=2) then '1'
				// else '0' end) as inCollection"
				// +
		",a.inCollection" + ",(case when a.createOn >=? then '1' else '0' end) as isnewst"
				+ ",(case when exists(select lip.id from LLicenseIp lip where (lip.license.publications.id=a.id or lip.license.publications.code=a.code) and lip.sip<=? and lip.eip>=? and lip.license.status=1) then '1' else '0' end) as IPRange";
		if (user != null) {
			field += ",(case when exists(select li.id from LLicense li where li.status=1 and li.user.id=? and (li.publications.id=a.id or li.publications.code=a.code)) then '1' else '0' end) as subscribedUser";
			field += ",(case when exists(select ll.id from LLicense ll where ll.status=2 and ll.user.id=? and (ll.publications.id=a.id or ll.publications.code=a.code)) then '1' else '0' end) as exLicense";
		...
		try {
			list = super.hibernateDao.getListByHql(property, field, hql + t.get("where").toString(), con.toArray(),
					sort, PPublications.class.getName(), pageCount, page * pageCount);
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<PPublications> getPagingList(Map<String, Object> condition, String sort, Integer pageCount,
			Integer page, CUser user, String ip) throws Exception {
		List<PPublications> list = null;
		String hql = " from PPublications a join a.publisher b ";
		// 求当前时间前的2周以前的日期
	...
	public List<PPublications> getSimplePagingList(Map<String, Object> condition, String sort, Integer pageCount,
		...
		String property = "sourceId,startVolume,endVolume,pdf,volume.id,issue.id,year,month,volumeCode,issueCode,chineseTitle,sisbn,hisbn,status,id,publisher.id,publisher.name,title,author,code,listPrice,lcurr,type"
				+ ",pubSubject,cover,createOn,updateOn,pdf,path,lang,remark,homepage,newest,selected,special,pubDate,startPage,endPage"
				+ ",oa,free,local,sisbn,hisbn,publisher.id,publisher.browsePrecent,publisher.downloadPercent,publisher.printPercent,publisher.journalBrowse,publisher.journalPrint,publisher.journalDownload";
		String field = "a.sourceId,a.startVolume,a.endVolume,a.pdf,a.volume.id,a.issue.id,a.year,a.month,a.volumeCode,a.issueCode,a.chineseTitle,a.sisbn,a.hisbn,a.status,a.id,b.id,b.name,a.title,a.author,a.code,a.listPrice,a.lcurr,a.type"
				+ ",a.pubSubject,a.cover,a.createOn,a.updateOn,a.pdf,a.path,a.lang,a.remark,a.homepage,a.newest,a.selected,a.special,a.pubDate,a.startPage,a.endPage"
				+ ",a.oa,a.free,a.local,a.sisbn,a.hisbn,b.id,b.browsePrecent,b.downloadPercent,b.printPercent,b.journalBrowse,b.journalPrint,b.journalDownload";
		try {
			list = super.hibernateDao.getListByHql(property, field, hql + t.get("where").toString(), con.toArray(),
					sort, PPublications.class.getName(), pageCount, page * pageCount);
		} catch (Exception e) {
	...
	public List<PPublications> getSimplePagingList(Map<String, Object> condition, String sort) throws Exception {
		...
		String property = "startVolume,endVolume,pdf,volume.id,issue.id,year,month,volumeCode,issueCode,chineseTitle,sisbn,hisbn,status,id,publisher.id,publisher.name,title,author,code,listPrice,lcurr,type"
				+ ",pubSubject,cover,createOn,updateOn,pdf,path,lang,remark,homepage,newest,selected,special,pubDate,startPage,endPage"
				+ ",oa,free,local,sisbn,hisbn,publisher.id,publisher.browsePrecent,publisher.downloadPercent,publisher.printPercent,publisher.journalBrowse,publisher.journalPrint,publisher.journalDownload";
		String field = "a.startVolume,a.endVolume,a.pdf,a.volume.id,a.issue.id,a.year,a.month,a.volumeCode,a.issueCode,a.chineseTitle,a.sisbn,a.hisbn,a.status,a.id,b.id,b.name,a.title,a.author,a.code,a.listPrice,a.lcurr,a.type"
				+ ",a.pubSubject,a.cover,a.createOn,a.updateOn,a.pdf,a.path,a.lang,a.remark,a.homepage,a.newest,a.selected,a.special,a.pubDate,a.startPage,a.endPage"
				+ ",a.oa,a.free,a.local,a.sisbn,a.hisbn,b.id,b.browsePrecent,b.downloadPercent,b.printPercent,b.journalBrowse,b.journalPrint,b.journalDownload";
		try {
			list = super.hibernateDao.getListByHql(property, field, hql + t.get("where").toString(), con.toArray(),
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<PPublications> getListIsNew(Map<String, Object> condition, String sort, Integer pageCount, Integer page)
			throws Exception {
		...
		try {
			list = super.hibernateDao.getListByHql(property, field, hql + t.get("where").toString(),
					((List<Object>) t.get("condition")).toArray(), sort, PPublications.class.getName(), pageCount,
					page * pageCount);
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
	...
	private Map<String, Object> getSubscriptionWhere(Map<String, Object> map) {
		 ...
		 * 2.licenseStatus
		 */
		if (CollectionsUtil.exist(map, "licenseStatus") && !"".equals(map.get("licenseStatus"))
				&& map.get("licenseStatus") != null) {
			if (flag == 0) {
				whereString += " where a.status = ?";
				flag = 1;
			} else {
		...
		if (CollectionsUtil.exist(map, "userId") && !"".equals(map.get("userId")) && map.get("userId") != null
				&& CollectionsUtil.exist(map, "institutionId") && !"".equals(map.get("institutionId"))
				&& map.get("institutionId") != null && CollectionsUtil.exist(map, "oafreeUid")
				&& !"".equals(map.get("oafreeUid")) && map.get("oafreeUid") != null) {
			if (flag == 0) {
			...
			condition.add(map.get("institutionId"));
			condition.add(map.get("level"));
		} else if (CollectionsUtil.exist(map, "oafreeUid") && !"".equals(map.get("oafreeUid"))
				&& map.get("oafreeUid") != null && CollectionsUtil.exist(map, "institutionId")
				&& !"".equals(map.get("institutionId")) && map.get("institutionId") != null) {
			if (flag == 0) {
				whereString += " where (b.id = ? or ( c.id = ? and b.level = ? ))";
				flag = 1;
			} else {
			...
			condition.add(map.get("level"));
				...
				&& map.get("oafreeUid") != null && CollectionsUtil.exist(map, "userId") && !"".equals(map.get("userId"))
				&& map.get("userId") != null) {
			if (flag == 0) {
			...
			condition.add(map.get("oafreeUid"));
		} else if (CollectionsUtil.exist(map, "institutionId") && !"".equals(map.get("institutionId"))
				&& map.get("institutionId") != null) {
			if (flag == 0) {
	...
	@SuppressWarnings("unchecked")
	public List<PPublications> getPagingList2(Map<String, Object> condition, String sort, Integer pageCount,
			Integer page, CUser user, String ip) throws Exception {
		List<PPublications> list = null;
		String hql = " from PPublications a left join a.publisher b left join a.publications p ";
		// 求当前时间前的2周以前的日期
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<PPublications> getSubscriptionPagingList(Map<String, Object> condition, String sort, Integer pageCount,
			Integer page, CUser user, String ip) throws Exception {
		List<PPublications> list = null;
		...
		property += ",recommand";
		String field = "d.volume.id,d.issue.id,d.year,d.month,d.volumeCode,d.issueCode,d.chineseTitle,d.sisbn,d.hisbn,d.status,d.id,d.publisher.id,d.publisher.name,d.title,d.author,d.code,d.listPrice,d.lcurr,d.browsePrecent,d.type"
				+ ",d.pubSubject,d.cover,d.createOn,d.updateOn,d.pdf,d.path,d.lang,d.remark,d.homepage,d.newest,d.selected,d.special,d.pubDate,d.startPage,d.endPage"
				+ ",d.oa,d.free,d.local" + ",d.startVolume" + ",d.endVolume" + ",d.inCollection"
				+ ",(case when d.createOn >=? then '1' else '0' end) as isnewst" +
				// ",'0'";
				",'1' as IPRange";
		if (user != null) {
	...
	public Integer getSubscriptionCount(Map<String, Object> condition) throws Exception {
		...
		String hql = " from LLicense a left join a.user b left join b.institution c left join a.publications d ";
		try {
			list = this.hibernateDao.getListByHql("id", "cast(count(*) as string)", hql + t.get("where").toString(),
					((List<Object>) t.get("condition")).toArray(), "", PPublications.class.getName());
		} catch (Exception e) {
			throw e;
		}
	...
	public List<PPublications> getTrialList(Map<String, Object> condition, String sort, Integer pageCount, Integer page)
		...
		String property = "startVolume,endVolume,pdf,volume.id,issue.id,year,month,volumeCode,issueCode,chineseTitle,sisbn,hisbn,status,id,publisher.id,publisher.name,title,author,code,listPrice,lcurr,type"
				+ ",pubSubject,cover,createOn,updateOn,pdf,path,lang,remark,homepage,newest,selected,special,pubDate,startPage,endPage"
				+ ",oa,free,local,sisbn,hisbn,publisher.id,publisher.browsePrecent,publisher.downloadPercent,publisher.printPercent,publisher.journalBrowse,publisher.journalPrint,publisher.journalDownload";
		String field = "d.startVolume,d.endVolume,d.pdf,d.volume.id,d.issue.id,d.year,d.month,d.volumeCode,d.issueCode,d.chineseTitle,d.sisbn,d.hisbn,d.status,d.id,e.id,e.name,d.title,d.author,d.code,d.listPrice,d.lcurr,d.type"
				+ ",d.pubSubject,d.cover,d.createOn,d.updateOn,d.pdf,d.path,d.lang,d.remark,d.homepage,d.newest,d.selected,d.special,d.pubDate,d.startPage,d.endPage"
				+ ",d.oa,d.free,d.local,d.sisbn,d.hisbn,e.id,e.browsePrecent,e.downloadPercent,e.printPercent,e.journalBrowse,e.journalPrint,e.journalDownload";
		try {
			list = super.hibernateDao.getListByHql(property, field, hql + t.get("where").toString(), con.toArray(),
					sort, PPublications.class.getName(), pageCount, pageCount * page);
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
		}
	...
	@SuppressWarnings("unchecked")
	public List<PPublications> getCountPagingList(Map<String, Object> condition, String sort, Integer pageCount,
			Integer page) throws Exception {
		List<PPublications> list = null;
		...
		try {
			list = this.hibernateDao.getListByHql(property, field, hql + t.get("where").toString(),
					((List<Object>) t.get("condition")).toArray(), sort, PPublications.class.getName(), pageCount,
					page * pageCount);
		} catch (Exception e) {
			throw e;
	...
	public void deleteByCondition(Map<String, Object> condition) throws Exception {
		...
		Map<String, Object> t = this.getWhere(condition);
		try {
			this.hibernateDao.executeHql(hql + t.get("where").toString(),
					((List<Object>) t.get("condition")).toArray());
		} catch (Exception e) {
			throw e;
		}
	}
	...
	public List<PPublications> getTypeStatistic(Map<String, Object> condition) throws Exception {
		...
		try {
			list = this.hibernateDao.getListByHql(property, field, hql + t.get("where").toString(),
					((List<Object>) t.get("condition")).toArray(), " group by a.type ", PPublications.class.getName(),
					5, 0);
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
	...
	public List<PPublications> getPublisherStatistic(Map<String, Object> condition) throws Exception {
		...
		String field = "(select c.name from BPublisher c where c.id=a.publisher.id) as publisher"
				+ ",cast(count(*) as string) as num,a.publisher.id"
				+ ",(select length(c.name) from BPublisher c where c.id=a.publisher.id) as pub_len"
				+ ",length(cast(count(*) as string)) as nm_len";
		try {
			list = this.hibernateDao.getListByHql(property, field, hql + t.get("where").toString(),
					((List<Object>) t.get("condition")).toArray(), " group by a.publisher.id order by publisher",
					PPublications.class.getName());
		} catch (Exception e) {
			throw e;
		}
	...
	public List<PPublications> getSubscriptionTypeStatistic(Map<String, Object> condition) throws Exception {
		...
		try {
			list = this.hibernateDao.getListByHql(property, field, hql + t.get("where").toString(),
					((List<Object>) t.get("condition")).toArray(), " group by d.type ", PPublications.class.getName(),
					5, 0);
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
	...
	public List<PPublications> getSubscriptionYearStatistic(Map<String, Object> condition) throws Exception {
		...
		try {
			list = this.hibernateDao.getListByHql(property, field, hql + t.get("where").toString(),
					((List<Object>) t.get("condition")).toArray(),
					" group by substring(d.pubDate,1,4) order by substring(d.pubDate,1,4) desc ",
					PPublications.class.getName());
		} catch (Exception e) {
			throw e;
		}
	...
	public List<PPublications> getSubscriptionPublisherStatistic(Map<String, Object> condition) throws Exception {
		...
		String hql = " from LLicense a left join a.user b left join b.institution c left join a.publications d ";
		String property = "publisher.name,id,publisher.id,startPage,inCollection";
		String field = "(select c.name from BPublisher c where c.id=d.publisher.id) as publisher"
				+ ",cast(count(*) as string) as num,d.publisher.id"
				+ ",(select length(c.name) from BPublisher c where c.id=d.publisher.id) as pub_len"
				+ ",length(cast(count(*) as string)) as nm_len";
		try {
			list = this.hibernateDao.getListByHql(property, field, hql + t.get("where").toString(),
					((List<Object>) t.get("condition")).toArray(), " group by d.publisher.id order by publisher",
					PPublications.class.getName());
		} catch (Exception e) {
			throw e;
		}
	...
	public List<PPublications> getPPublicationsByISBN(String isbn) throws Exception {
		...
		try {
			list = this.hibernateDao.getListByHql(property, field, hql + t.get("where").toString(),
					((List<Object>) t.get("condition")).toArray(), null, PPublications.class.getName());
		} catch (Exception e) {
			throw e;
		}
		return list;
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<PPublications> getHots(Map<String, Object> condition, String sort, CUser user, String ip)
			throws Exception {
		List<PPublications> list = null;
		String hql = " from PPublications a left join a.publisher b  ";
		Map<String, Object> t = this.getWhere(condition);
		...
		String property = "authorAlias,chineseTitle,pubSubjectEn,sisbn,hisbn,status,publisher.name,id,publisher.id,publisher.name,title,author,code,listPrice,lcurr,browsePrecent,type"
				+ ",pubSubject,cover,createOn,updateOn,pdf,path,lang,remark,homepage,newest,selected,special,pubDate,startPage,endPage"
				+ ",oa,free,local" +
				// ",allSubjectName" +
		...
		property += ",recommand,buyTimes";
		String field = "a.author,a.chineseTitle,a.pubSubjectEn,a.sisbn,a.hisbn,a.status,b.name,a.id,b.id,b.name,a.title,a.author,a.code,a.listPrice,a.lcurr,a.browsePrecent,a.type"
				+ ",a.pubSubject,a.cover,a.createOn,a.updateOn,a.pdf,a.path,a.lang,a.remark,a.homepage,a.newest,a.selected,a.special,a.pubDate,a.startPage,a.endPage"
				+ ",a.oa,a.free,a.local" +
	...
	@SuppressWarnings("unchecked")
	public List<PPublications> getSimplePubList(Map<String, Object> condition, String sort, Integer number)
			throws Exception {
		List<PPublications> list = null;
		String hql = " from PPublications a ";
		Map<String, Object> t = this.getWhere(condition);
		...
		if (condition.containsKey("justYear")) {
			...
			property = "id,title,code,volume.id,volumeCode,issue.id,issueCode,year,month,pdf,startPage,journalOrder";
			field = "a.id,a.title,a.code,a.volume.id,a.volumeCode,a.issue.id,a.issueCode,a.year,a.month,a.pdf,a.startPage,a.journalOrder";
		}
		try {
			if (number > 0) {
				list = this.hibernateDao.getListByHql(property, field, hql + t.get("where").toString(),
						((List<Object>) t.get("condition")).toArray(), sort, PPublications.class.getName(), number, 0);
			} else {
				list = this.hibernateDao.getListByHql(property, field, hql + t.get("where").toString(),
						((List<Object>) t.get("condition")).toArray(), sort, PPublications.class.getName());
			}
	...
	@SuppressWarnings("unchecked")
	public List<PPublications> getSimplePageList(Map<String, Object> condition, String sort, Integer pageCount,
			Integer page) throws Exception {
		List<PPublications> list = null;
		String hql = " from PPublications a ";
		Map<String, Object> t = this.getWhere(condition);
	...
	public List<PPublications> getArticleList(Map<String, Object> condition, String sort, CUser user, String ip)
		...
		String property = "available,treecode,journalType,periodicType,volume.id,issue.id,year,month,volumeCode,issueCode,chineseTitle,sisbn,hisbn,status,id,publisher.id,publisher.name,title,author,code,listPrice,lcurr,browsePrecent,type"
				+ ",pubSubject,cover,createOn,updateOn,pubDate,pdf,path,lang,remark,homepage,newest,selected,special,pubDate,startPage,endPage"
				+ ",oa,free,local" + ",startVolume" + ",endVolume" + ",inCollection,latest,subscribedIp"
				+ ",keywords,reference,doi" + ",publications.id,publications.code,publications.title,eissn,journalOrder";
		if (user != null) {
			property += ",subscribedUser,exLicense,buyInDetail,favorite";
		}
		property += ",recommand";
		String field = "a.available,a.treecode,a.journalType,a.periodicType,a.volume.id,a.issue.id,a.year,a.month,a.volumeCode,a.issueCode,a.chineseTitle,a.sisbn,a.hisbn,a.status,a.id,b.id,b.name,a.title,a.author,a.code,a.listPrice,a.lcurr,a.browsePrecent,a.type"
				+ ",a.pubSubject,a.cover,a.createOn,a.updateOn,a.pubDate,a.pdf,a.path,a.lang,a.remark,a.homepage,a.newest,a.selected,a.special,a.pubDate,a.startPage,a.endPage"
				+ ",a.oa,a.free,a.local" + ",a.startVolume" + ",a.endVolume" +
				// ",(select max(concat(t.year,'-',t.volumeCode)) from
				// PPublications t where t.publications.id=a.id and t.type=6 )"
				// +
		...
		",a.inCollection"
				...
				// ",'0'";
				",(case when exists(select lip.id from LLicenseIp lip where (lip.license.publications.id=a.id or lip.license.publications.code=a.code) and lip.sip<=? and lip.eip>=? and lip.license.status=1) then '1' else '0' end) as IPRange"
				+ ",a.keywords,a.reference,a.doi" + ",c.id,c.code,c.title,a.eissn,a.journalOrder";
	...
	public List<PPublications> getArticlePagingList(Map<String, Object> condition, String sort, Integer pageCount,
		...
		String property = "publications.id,publications.journalOrder,publications.code,volume.id,issue.id,year,month,volumeCode,issueCode,chineseTitle,sisbn,hisbn,status,id,publisher.id,publisher.name,title,author,code,listPrice,lcurr,browsePrecent,type"
				+ ",journalType,periodicType,pubSubject,cover,createOn,updateOn,pdf,path,lang,remark,homepage,newest,selected,special,pubDate,startPage,endPage"
				+ ",oa,free,local" + ",startVolume" + ",endVolume" + ",inCollection,latest,subscribedIp"
				+ ",keywords,reference,doi" + ",publications.id,publications.code,publisher.journalDownload";
		if (user != null) {
			property += ",subscribedUser,exLicense,buyInDetail,favorite";
		}
		property += ",recommand";
		String field = "a.publications.id,a.journalOrder,a.publications.code,a.volume.id,a.issue.id,a.year,a.month,a.volumeCode,a.issueCode,a.chineseTitle,a.sisbn,a.hisbn,a.status,a.id,b.id,b.name,a.title,a.author,a.code,a.listPrice,a.lcurr,a.browsePrecent,a.type"
				+ ",a.journalType,a.periodicType,a.pubSubject,a.cover,a.createOn,a.updateOn,a.pdf,a.path,a.lang,a.remark,a.homepage,a.newest,a.selected,a.special,a.pubDate,a.startPage,a.endPage"
				+ ",a.oa,a.free,a.local" + ",a.startVolume" + ",a.endVolume" +
				// ",(select max(concat(t.year,'-',t.volumeCode)) from
				// PPublications t where t.publications.id=a.id and t.type=6 )"
				// +
		...
		",a.inCollection"
				...
				// ",'0'";
				",(case when exists(select lip.id from LLicenseIp lip where (lip.license.publications.id=a.id or lip.license.publications.code=a.code) and lip.sip<=? and lip.eip>=? and lip.license.status=1) then '1' else '0' end) as IPRange"
				+ ",a.keywords,a.reference,a.doi" + ",c.id,c.code,b.journalDownload";
		if (user != null) {
			field += ",(case when exists(select li.id from LLicense li where li.status=1 and li.user.id=? and (li.publications.id=a.id or li.publications.code=a.code)) then '1' else '0' end) as subscribedUser";
			field += ",(case when exists(select ll.id from LLicense ll where ll.status=2 and ll.user.id=? and (ll.publications.id=a.id or ll.publications.code=a.code)) then '1' else '0' end) as exLicense";
			field += ",(case when exists(select detail.id from OOrderDetail detail where detail.status not in (3,10,99) and detail.user.id=? and (detail.price.publications.id=a.id or detail.price.publications.code=a.code)) then '1' else '0' end) as buyInDetail";// 2013-3-13
	...
	public List<PPublications> getDatabaseList(Map<String, Object> condition, String sort, CUser user, String ip)
		...
		String property = "volume.id,issue.id,year,month,volumeCode,issueCode,chineseTitle,sisbn,hisbn,status,id,publisher.id,publisher.name,title,author,code,listPrice,lcurr,browsePrecent,type"
				+ ",pubSubject,cover,createOn,updateOn,pdf,path,lang,remark,homepage,newest,selected,special,pubDate,startPage,endPage"
				+ ",oa,free,local,inCollection,latest,subscribedIp";
		if (user != null) {
			property += ",subscribedUser,exLicense,buyInDetail,favorite";
		}
		property += ",recommand";
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<PPublications> getDatabasePagingList(Map<String, Object> condition, String sort, Integer pageCount,
			Integer page, CUser user, String ip) throws Exception {
		List<PPublications> list = null;
		String hql = " from PPublications a left join a.publisher b ";
		// 求当前时间前的2周以前的日期
	...
	public String getStartVolume(String id) throws Exception {
		...
		try {
			...
			List<PPublications> list = this.hibernateDao.getListByHql("id", "min(concat(a.year,'-',a.volumeCode))", hql,
					new Object[] { id }, "", PPublications.class.getName());
	...
	public String getEndVolume(String id) throws Exception {
		...
		try {
			String hql = " from PPublications a where a.publications.id=? and a.type=6 ";
			List<PPublications> list = this.hibernateDao.getListByHql("id", "max(concat(a.year,'-',a.volumeCode))", hql,
					new Object[] { id }, "", PPublications.class.getName());
			if (list != null && list.size() > 0) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<PPublications> getLicensePagingList(Map<String, Object> condition, String sort, Integer pageCount,
			Integer page, CUser user, String ip) throws Exception {
		List<PPublications> list = null;
		...
		List<Object> con = new ArrayList<Object>();
		con.add(cn.com.daxtech.framework.util.DateUtil.toDate(this.getDistryDate(), "yyyy-MM-dd"));
		if (CollectionsUtil.exist(condition, "institutionId") && condition.get("institutionId") != null
				&& !"".equals(condition.get("institutionId"))) {
			con.add(IpUtil.getLongIp(ip));
			con.add(IpUtil.getLongIp(ip));
		}
		if (user != null) {
		...
		String property = "year,month,chineseTitle,status,id,publisher.id,publisher.name,title,author,code,type"
				+ ",cover,remark,homepage,newest,selected,special,pubDate" + ",oa,free,local" + ",startVolume"
				+ ",endVolume" + ",inCollection,latest,subscribedIp";
		if (user != null) {
			property += ",subscribedUser,exLicense,buyInDetail,favorite";
		}
		property += ",recommand";
		String field = "a.year,a.month,a.chineseTitle,a.status,a.id,b.id,b.name,a.title,a.author,a.code,a.type"
				+ ",a.cover,a.remark,a.homepage,a.newest,a.selected,a.special,a.pubDate" + ",a.oa,a.free,a.local"
				+ ",a.startVolume" + ",a.endVolume" +
				// ",(case when exists(select cc.id from PCcRelation cc where
				// cc.publications.id=a.id and cc.collection.status=2 ) then '1'
				// else '0' end) as inCollection"
				// +
		",a.inCollection"
				+ ",(case when exists(select pub.id from PPublications pub where pub.id=a.id and pub.createOn >=?  ) then '1' else '0' end) as isnewst";
		if (CollectionsUtil.exist(condition, "institutionId") && condition.get("institutionId") != null
				&& !"".equals(condition.get("institutionId"))) {
		 ...
		 * license
		 */
		if (CollectionsUtil.exist(condition, "license") && condition.get("license") != null
				&& !"".equals(condition.get("license"))) {
		...
		try {
			list = super.hibernateDao.getListByHql(property, field, hql + _hql, con.toArray(), sort,
					PPublications.class.getName(), pageCount, page * pageCount);
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
		}
	...
	public Integer getLicenseCount(Map<String, Object> condition) throws Exception {
		...
		if (CollectionsUtil.exist(condition, "license") && condition.get("license") != null
			...
			if (CollectionsUtil.exist(condition, "userId") && condition.get("userId") != null
					&& !"".equals(condition.get("userId"))) {
		...
		try {
			list = this.hibernateDao.getListByHql("id", "cast(count(*) as string)",
					hql + t.get("where").toString() + _hql, con.toArray(), "", PPublications.class.getName());
		} catch (Exception e) {
			throw e;
		}
		return list == null ? 0 : Integer.valueOf(list.get(0).getId());
	...
	public List<PPublications> getTypeCount(Map<String, Object> condition) throws Exception {
		...
		try {
			list = super.hibernateDao.getListByHql(property, field, hql + t.get("where").toString(), o2,
					" group by a.type ", PPublications.class.getName());
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<PPublications> getList3(Map<String, Object> condition, String sort, CUser user, String ip)
			throws Exception {
		List<PPublications> list = null;
		String hql = " from PPublications a left join a.publisher b left join a.publications p ";
		Map<String, Object> t = this.getWhere(condition);
		...
		String property = " sourceId,fileType,periodicType,journalType,publications.id,publications.title,publisher.id,publisher.name,publisher.nameEn,publisher.probationPer,publisher.probationValue,id,title,chineseTitle,author,code,browsePrecent,type,year,pubDate,remark,available,pdf,cover,browsePrecent,startPage,endPage"
				+ ",authorAlias,oa,free,local" + ",startVolume" + ",endVolume" + ",inCollection,latest,subscribedIp";
		if (user != null) {
			property += ",subscribedUser,exLicense,buyInDetail,favorite";
		}
		property += ",recommand,probation ";
		String field = " a.sourceId,a.fileType,a.periodicType,a.journalType,p.id,p.title,b.id,b.name,b.nameEn,b.probationPer,b.probationValue,a.id,a.title,a.chineseTitle,a.author,a.code,a.browsePrecent,a.type,a.year,a.pubDate,a.remark,a.available,a.pdf,a.cover,a.browsePrecent,a.startPage,a.endPage"
				+ ",a.author,a.oa,a.free,a.local" + ",a.startVolume" + ",a.endVolume" + ",a.inCollection"
				+ ",(case when exists(select pub.id from PPublications pub where pub.createOn >=? and pub.id=a.id) then '1' else '0' end) as isnewst"
				+ ",(case when exists(select lll.id from LLicense lll where (lll.publications.id=a.id or lll.publications.code=a.code) and lll.status=1 and "
				+ "(exists(select ipr.id from BIpRange ipr where ipr.institution.id=lll.user.institution.id and ipr.sip<=? and ipr.eip>=?) and lll.user.level=2"
				+ " or "
				+ "exists(select lip.id from LLicenseIp lip where lip.license.id=lll.id and lip.sip<=? and lip.eip>=?) and lll.user.level=2)) then '1' else '0' end) as IPRange";
		if (user != null) {
			field += ",(case when exists(select li.id from LLicense li where li.status=1 and li.user.id=? and (li.publications.id=a.id or li.publications.code=a.code)) then '1' else '0' end) as subscribedUser";
			field += ",(case when exists(select ll.id from LLicense ll where ll.status=2 and ll.user.id=? and (ll.publications.id=a.id or ll.publications.code=a.code)) then '1' else '0' end) as exLicense";
			field += ",(case when exists(select detail.id from OOrderDetail detail where detail.status not in (3,10,99) and detail.user.id=? and (detail.price.publications.id=a.id or detail.price.publications.code=a.code)) then '1' else '0' end) as buyInDetail";// 2013-3-13
	...
	public void updateJournalp(Map<String, Object> vals, String[] ids) throws Exception {
		try {
			...
			if (ids != null && vals != null && !vals.isEmpty()) {
				...
				if (!"".equals(where.trim()) && !"".equals(set.trim())) {
					String hql = "update PPublications a  set a.homepage = 2," + set + " where a.publications.id in ("
							+ where + ")";
					this.hibernateDao.executeHql(hql, condition.toArray());
				}
			}
	...
	public PPublications getChapter(String pubId, int pageNum) throws Exception {
		...
		try {
			...
			List<PPublications> list = this.hibernateDao.getListByHql(property, field, hql + whereString, con.toArray(),
					"", PPublications.class.getName());
			if (list != null && list.size() > 0) {
				pub = list.get(0);
			}
	...
	public List<PPublications> getpublisherInfoForCol(Map<String, Object> condition, String sort) throws Exception {
		...
		String field = "a.publisher.id,max(a.publisher.name) AS NAME_CLU,max(a.publisher.nameEn) AS NAMEEN_CLU,cast(count(*) as string)";
		String hql = " from PPublications a where exists(select 1 from PCcRelation b where a.id=b.publications.id and b.collection.id='"
				+ condition.get("collectionId") + "'";
		if (CollectionsUtil.exist(condition, "language") && !"".equals(condition.get("language"))
				&& condition.get("language") != null) {
			hql += " and b.publications.lang like 'chs'";
		}
		if (CollectionsUtil.exist(condition, "languageEn") && !"".equals(condition.get("languageEn"))
				&& condition.get("languageEn") != null) {
			hql += " and b.publications.lang not like 'chs'";
		}
		hql += ") group by a.publisher.id";
		try {
			list = super.hibernateDao.getListByHql(property, field, hql, ((List<Object>) t.get("condition")).toArray(),
					sort, PPublications.class.getName());
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
	...
	public List<PPublications> getBuyTimesInfo(String subjectId, String sort, Integer pubNum) throws Exception {
		...
		try {
			list = super.hibernateDao.getListByHql(property, field, hql, condition, sort, PPublications.class.getName(),
					pubNum, 0);
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
	...
	@SuppressWarnings("unchecked")
	public List<PPublications> getNewestPublications(Map<String, Object> condition, String sort, long count)
			throws Exception {
		List<PPublications> list = null;
		String hql = " FROM PPublications a left join a.publisher b ";
		...
		try {
			list = hibernateDao.getListByHql(property, field,
					hql + t.get("where").toString() + " AND ROWNUM <= " + count,
					((List<Object>) t.get("condition")).toArray(), sort, PPublications.class.getName());
		} catch (Exception e) {
			throw e;
		}
	...
	public List<PPublications> getPagingListNew(Map<String, Object> condition, String sort, Integer pageCount,
			Integer page) throws Exception {
		List<PPublications> list = null;
		Map<String, Object> t = this.getWhere(condition);
	...
	private Map<String, Object> getTrialWhere(Map<String, Object> map) {
		 ...
		 * 资源类型
		 */
		if (CollectionsUtil.exist(map, "trialType") && !"".equals(map.get("trialType"))
				&& map.get("trialType") != null) {
			if (flag == 0) {
		...
		// 首页是否为试用资源
		if (CollectionsUtil.exist(map, "myisTrial") && !"".equals(map.get("myisTrial"))
				&& map.get("myisTrial") != null) {
			if (flag == 0) {
		...
		if (CollectionsUtil.exist(map, "trypublisherName") && !"".equals(map.get("trypublisherName"))
				&& map.get("trypublisherName") != null) {
			if (flag == 0) {
