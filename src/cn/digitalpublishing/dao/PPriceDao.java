...
public class PPriceDao extends CommonDao<PPrice, String> {
	...
	private Map<String,Object> getWhere(Map<String,Object> map){
		...
		if(CollectionsUtil.exist(map, "category")&&!"".equals(map.get("category"))){
			if(flag==0){
				whereString+=" where a.category = ?";
				flag=1;
			}else{
				whereString+=" and a.category = ?";
			}
			condition.add(map.get("category"));
		}
		if(CollectionsUtil.exist(map, "pricetype")&&!"".equals(map.get("pricetype"))){
			if(flag==0){
			...
			condition.add(map.get("pricetype").toString());
		}
