...
public class BIpRangeDao extends CommonDao<BIpRange,String> {
	...
	private Map<String,Object> getWhere(Map<String,Object> map){
		...
		if(CollectionsUtil.exist(map, "startip")&&!"".equals(map.get("startip"))){
			if(flag==0){
				whereString+=" where a.startIp = ? ";
				flag=1;
			}else{
				whereString+=" and a.startIp = ? ";
			}
			condition.add(map.get("startip").toString());
		}
		if(CollectionsUtil.exist(map, "endip")&&!"".equals(map.get("endip"))){
			if(flag==0){
				whereString+=" where a.endIp = ? ";
				flag=1;
			}else{
				whereString+="  and a.endIp = ? ";
			}
			condition.add(map.get("endip").toString());
		}
		/**
		 * institutionId
		 */
