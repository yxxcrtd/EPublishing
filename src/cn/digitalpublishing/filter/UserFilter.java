				...
				if(session.getAttribute("isFristRequest")==null || "yes".equals(session.getAttribute("isFristRequest"))){//第一次请求判断：如果是
					...
					if(ipList!=null&&!ipList.isEmpty()){//IP范围内判断：如果在IP范围内，设置成默认订阅范围内
						...
						if(users!=null && users.size()>0){
							...
							//若用户所属机构的信息中包含logo，取出					
							String logo=null;
							if( ipRange.getInstitution()!=null &&  ipRange.getInstitution().getLogo()!=null){						
								Boolean hasurl=false,hasnote=false;
								logo="";
								if(ipRange.getInstitution().getLogoUrl()!=null && !"".equals(ipRange.getInstitution().getLogoUrl().trim()) ){
									hasurl=true;
									logo+="<a target=\"_blank\" href=\"" + ipRange.getInstitution().getLogoUrl()+  "\" >";
								}
								if(ipRange.getInstitution().getLogoNote()!=null && !"".equals(ipRange.getInstitution().getLogoNote().trim()) ){
									hasnote=true;
								}
								logo+="<img style=\"width:187px;height:35px; margin-bottom:13px;\"  src=\"" +(ctx!=null&&!"".equals(ctx)?ctx:"http://"+domain)+ ipRange.getInstitution().getLogo() + "\" title=\""+ (hasnote ?ipRange.getInstitution().getLogoNote():"")+"\"/>";
								logo+=(hasurl?"</a>":"") ;
