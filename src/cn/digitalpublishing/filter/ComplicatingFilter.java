		...
		public void doFilter(ServletRequest request, ServletResponse response,FilterChain chin) throws IOException, ServletException {
			try {
				...
				if(path.contains("/pages/complication/close")){
					...
					/*List<LLicenseIp> list = this.serviceFactory.getPPublicationsService().getLicenseIpList(condition,"");
					if(list!=null&&list.size()>0){
						for(LLicenseIp licenseIp:list){
							LLicense license = licenseIp.getLicense();
							if(license.getComplicating()!=null&&license.getComplicating()!=0){//即有并发限制
								...
								this.serviceFactory.getPPublicationsService().deleteComplicatingByCondition(uCon);
							}
						}
					}else{
						...
						if(user!=null){
							...
							List<LLicense> list2 = this.serviceFactory.getPPublicationsService().getLicenseList(map2,"");
							if(list2!=null&&list2.size()>0){
								for (LLicense license : list2) {
									if(license.getComplicating()!=null&&license.getComplicating()!=0){//即有并发限制
										Map<String,Object> uCon = new HashMap<String, Object>();
										uCon.put("licenseId", license.getId());
										uCon.put("macId", md5Str);
