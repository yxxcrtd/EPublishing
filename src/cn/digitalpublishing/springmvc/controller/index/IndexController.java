...
public class IndexController extends BaseController {
	 ...
	 * @throws Exception
	 */
	@RequestMapping("index/probation")
	public ModelAndView probation(HttpServletRequest request, HttpServletResponse response, IndexForm form) throws Exception{
		String forwardString = "index/probation";
		Map<String, Object> model = new HashMap<String, Object>();
		CUser user = request.getSession().getAttribute("mainUser") == null ? null : (CUser) request.getSession().getAttribute("mainUser");
		BInstitution ins = (BInstitution) request.getSession().getAttribute("institution");
		try {
			form.setUrl(request.getRequestURL().toString());
			String keyword = CharUtil.toSimple("1");//查询试读 1-试读
			String lang = (String) request.getSession().getAttribute("lang");//语言
			
			Map<String, String> param = new HashMap<String, String>();//solr参数
			...
			if (keyword != null && !"".equals(keyword)) {
					...
					resultMap = this.probationfullTextIndexService.searchByAllFullText(keyword, form.getCurpage(), form.getPageCount(), param, form.getSearchOrder());
	...
	public ModelAndView search(HttpServletRequest request, HttpServletResponse response, IndexForm form) throws Exception {
		...
		try {
			...
			form.setSearchsType(null == form.getSearchsType() ? 
					Integer.valueOf(null == request.getParameter("searchsType2") || "".equals(request.getParameter("searchsType2")) ? 
							request.getParameter("type") : request.getParameter("searchsType2")) 
						: (null == form.getSearchsType() || "".equals(form.getSearchsType()) ? Integer.valueOf(request.getParameter("type")) : form.getSearchsType()));
		...
		try {
			...
			if (user != null && (user.getLevel() == 2 || user.getLevel() == 5)) {
				...
				if (keyword != null && !"".equals(keyword)) {
					...
					// 在solr中查询 [搜索类型=====0-全文;1-标题;2-作者]
					Map<String, Object> resultMap = new HashMap<String, Object>();
					form.setSearchsType(form.getSearchsType() == null ? 0 : form.getSearchsType());
					if (form.getIsAccurate() != null && form.getIsAccurate() == 2) {// 要查询的内容
																					// 是否精确查找
																					// 1、否
																					// ；2、是
	...
	public ModelAndView free(HttpServletRequest request, HttpServletResponse response, IndexForm form) throws Exception {
		...
		try {
			...
			if ("true".equals(allfree) && !allfree.equals("null") && allfree != "null") { // condition.put("author",
				...
				condition.put("noChapter", "3"); // 排除“章节”资源类型
				...
				for (PPublications pPublications : lists) {
					String ppName = "" ; 
					try{
						ppName = pPublications.getPublisher().getName();
					}catch(NullPointerException e){
						System.out.println("-------------------------" + pPublications.getId()) ; 
					}
	...
	public String searchAllCountForAjax(HttpServletRequest request, HttpServletResponse response, IndexForm form) throws Exception {
		...
		try {
			...
			String searchsType2 = request.getParameter("searchsType2");
			searchsType2 = getValus(searchsType2);
			form.setSearchsType(form.getSearchsType() == null ? Integer.valueOf(searchsType2) : form.getSearchsType());
			// String keyword = form.getSearchValue();
			...
			/*
			 * form.setSearchsType(form.getSearchsType() == null ||
			 * "".equals(form.getSearchsType()) ? 0 : form.getSearchsType());
			 */
			if (keyword != null && !"".equals(keyword)) {
				switch (form.getSearchsType()) {
				case 0:
					resultMap = this.publicationsIndexService.searchByAllFullText(keyword, 0, form.getPageCount(), param, form.getSearchOrder());
					break;
				case 1:
					resultMap = this.publicationsIndexService.searchByTitle(keyword, 0, form.getPageCount(), param, form.getSearchOrder());
					break;
				case 2:
					resultMap = this.publicationsIndexService.searchByAuthor(keyword, 0, form.getPageCount(), param, form.getSearchOrder());
					break;
				case 3:
					resultMap = this.publicationsIndexService.searchByISBN(keyword, 0, form.getPageCount(), param, form.getSearchOrder());
					break;
				case 4:
					resultMap = this.publicationsIndexService.searchByPublisher(keyword, 0, form.getPageCount(), param, form.getSearchOrder());
					break;
				default:
					resultMap = this.publicationsIndexService.searchByAllFullText(keyword, 0, form.getPageCount(), param, form.getSearchOrder());
					break;
