...
public class BPublisherController extends BaseController {
	...
	public void insertSource(HttpServletRequest request, Model model){
		ResultObject<EBSource> result=null;
		try{
			String objJson = request.getParameter("obj").toString();
			String operType = request.getParameter("operType").toString(); //1-insert 2-update
			Converter<EBSource> converter=new Converter<EBSource>();
			EBSource obj=(EBSource)converter.json2Object(objJson, EBSource.class.getName());
			if("2".equals(operType)){
				this.configureService.updateEBsource(obj, obj.getId(), null);
			}else if ("1".equals(operType)){
				this.configureService.insertEBsource(obj);
			}else{
				//删除
			}
			ObjectUtil<EBSource> util=new ObjectUtil<EBSource>();
			obj=util.setNull(obj, new String[]{Set.class.getName()});
			result=new ResultObject<EBSource>(1,obj,Lang.getLanguage("publisher.info.maintain.success",request.getSession().getAttribute("lang").toString()));//"出版商信息维护成功！");
		}catch(Exception e){
			result=new ResultObject<EBSource>(2,Lang.getLanguage("publisher.info.maintain.error",request.getSession().getAttribute("lang").toString()));//"出版商信息维护失败！");
		}
		model.addAttribute("target",result);
	}
