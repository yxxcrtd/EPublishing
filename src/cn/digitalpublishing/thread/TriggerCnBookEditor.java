	...
	private void tigger() {
		...
		try {
			...
			for (BInstitution ins : binsList) {
				...
				editorRecommendsList = serviceFactory.getLogAOPService().getLogOfHotReadingForRecommad(ucMap,
				...
				if (editorRecommendsList.size() >= 24) {
					...
					for (LAccess a : editorRecommendsList) {
						bookDao.zadd(ins.getId() + "__Editor__CN",
					...
					List<String> cnBookEditor = bookDao.getSet("cnBookEditor");
					if (cnBookEditor.size() == 0) {
						ucMap.remove("pInsId");
						ucMap.remove("noLicense");
						editorRecommendsList = serviceFactory.getLogAOPService().getLogOfHotReadingForRecommad(ucMap,
								" group by b.id order by count(a.id) desc ", 24, 0);
						for (LAccess a : editorRecommendsList) {
							bookDao.zadd("cnBookEditor",
									a.getPublications().getId() + a.getPublications().getTitle() + "##author##"
											+ a.getPublications().getAuthor() + "##publisher##"
											+ a.getPublications().getPublisher().getName() + "##cover##"
											+ a.getPublications().getCover());
