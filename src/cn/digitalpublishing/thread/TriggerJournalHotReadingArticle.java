	...
	private void tigger() {
		...
		try {
			...
			for (BInstitution ins : binsList) {
				...
				List<LAccess> hotlist = serviceFactory.getLogAOPService().getJournalHotReading(hotcondition,
						" group by a.publications.id order by count(*) desc ", 12, 0);

				if (hotlist != null && !hotlist.isEmpty() && hotlist.size() >= 12) {
					for (LAccess aa : hotlist) {
						PPublications p = serviceFactory.getPPublicationsService()
								.findByIdNew(aa.getPublications().getId());
						bookDao.zadd(ins.getId() + "__JournalHotReadingArticle",
								p.getId() + p.getTitle() + "##parentId##" + p.getPublications().getId()
										+ "##parentTitle##" + p.getPublications().getTitle() + "##year##" + p.getYear()
										+ "##volumeCode##" + p.getVolumeCode() + "##issueCode##" + p.getIssueCode()
										+ "##startPage##" + p.getStartPage() + "##endPage##" + p.getEndPage()
										+ "##publisher##" + p.getPublisher().getName());

					}
				} else {
					List<String> recommendList = bookDao.getSet("JournalHotReadingArticle");
					if (recommendList.size() == 0) {
						hotcondition.remove("institutionId");
						hotcondition.remove("license");// IP范围内时仅显示license有效的出版物，ip范围外不考虑license是否有效
						hotlist = serviceFactory.getLogAOPService().getJournalHotReading(hotcondition,
								" group by a.publications.id order by count(*) desc ", 12, 0);
						for (LAccess aa : hotlist) {

							PPublications p = serviceFactory.getPPublicationsService()
									.findByIdNew(aa.getPublications().getId());
							System.out.println(p.getId() + p.getTitle() + "##parentId##" + p.getPublications().getId()
									+ "##parentTitle##" + p.getPublications().getTitle() + "##year##" + p.getYear()
									+ "##volumeCode##" + p.getVolumeCode() + "##issueCode##" + p.getIssueCode()
									+ "##startPage##" + p.getStartPage() + "##endPage##" + p.getEndPage()
									+ "##publisher##" + p.getPublisher().getName());
							bookDao.zadd("JournalHotReadingArticle",
									p.getId() + p.getTitle() + "##parentId##" + p.getPublications().getId()
											+ "##parentTitle##" + p.getPublications().getTitle() + "##year##"
											+ p.getYear() + "##volumeCode##" + p.getVolumeCode() + "##issueCode##"
											+ p.getIssueCode() + "##startPage##" + p.getStartPage() + "##endPage##"
											+ p.getEndPage() + "##publisher##" + p.getPublisher().getName());
