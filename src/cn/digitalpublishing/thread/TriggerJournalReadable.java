		...
		try {
			...
			if (list.size() >= 24) {
				...
				for (PPublications p : list) {
					System.out.println("可读资源------------>" + p.getId() + p.getTitle() + "##oa##" + p.getOa()
							+ "##free##" + p.getFree() + "##publisher##" + p.getPublisher().getName() + "##start##"
							+ p.getStartVolume() + "##end##" + p.getEndVolume());
					bookDao.zadd("journalReadbleList",
							p.getId() + p.getTitle() + "##oa##" + p.getOa() + "##free##" + p.getFree() + "##publisher##"
									+ p.getPublisher().getName() + "##start##" + p.getStartVolume() + "##end##"
									+ p.getEndVolume());
				}
			}
