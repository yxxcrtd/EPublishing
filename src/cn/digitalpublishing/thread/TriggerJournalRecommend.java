			...
			for (BInstitution ins : binsList) {
				...
				if (list.size() >= 24) {
					...
					for (LAccess a : list) {
						...
						bookDao.zadd(ins.getId() + "__JournalRecommend",
								a.getPublications().getId() + a.getPublications().getTitle() + "##publisher##"
										...
										+ a.getPublications().getOa() + "##free##" + a.getPublications().getFree());
					}

				} else {
					List<String> recommendList = bookDao.getSet("JournalRecommend");
