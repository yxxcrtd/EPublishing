	...
	private void tigger() {
		...
		try {
			...
			for (BInstitution ins : binsList) {
				...
				List<LAccess> hotlist = serviceFactory.getLogAOPService().getLogOfHotReading(hotcondition,
						" group by a.publications.id order by count(*) desc ", 18, 0);
				if (enHotReadingList.size() >= 12) {
					bookDao.delete(ins.getId() + "__enbookList__HotReading__EN");
					for (LAccess a : enHotReadingList) {
						bookDao.zadd(ins.getId() + "__enbookList__HotReading__EN",
								a.getPublications().getId() + a.getPublications().getTitle() + "##author##"
										+ a.getPublications().getAuthor() + "##publisher##"
										+ a.getPublications().getPublisher().getName() + "##cover##"
										+ a.getPublications().getCover());
					}

				} else {
					List<String> enHotReading = bookDao.getSet("enbookList__HotReading__EN");
					if (enHotReading.size() == 0) {
						hotcondition.remove("institutionId");
						hotcondition.remove("license");
						enHotReadingList = serviceFactory.getLogAOPService().getLogOfHotReading(hotcondition,
								" group by a.publications.id order by count(*) desc ", 18, 0);
						for (LAccess a : enHotReadingList) {
							bookDao.zadd("enbookList__HotReading__EN",
									a.getPublications().getId() + a.getPublications().getTitle() + "##author##"
											+ a.getPublications().getAuthor() + "##publisher##"
											+ a.getPublications().getPublisher().getName() + "##cover##"
											+ a.getPublications().getCover());
