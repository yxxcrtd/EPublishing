		...
		try {
			...
			for (BInstitution ins : binsList) {
				...
				if (cnHotReadingList.size() >= 24) {
					...
					List<String> enHotReading = bookDao.getSet("cnbookList__HotReading__CN");
					if (enHotReading.size() == 0) {
						hotcondition.remove("institutionId");
						hotcondition.remove("license");
						cnHotReadingList = serviceFactory.getLogAOPService().getLogOfHotReading(hotcondition,
								" group by a.publications.id order by count(*) desc ", 24, 0);
						for (LAccess a : cnHotReadingList) {
							bookDao.zadd("cnbookList__HotReading__CN",
									a.getPublications().getId() + a.getPublications().getTitle() + "##author##"
											+ a.getPublications().getAuthor() + "##publisher##"
											+ a.getPublications().getPublisher().getName() + "##cover##"
											+ a.getPublications().getCover());
